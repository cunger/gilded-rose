...
class GildedRose {
    ...
    public void updateQuality() {
        for (int i = 0; i < items.length; i++) {
            ...
            final int qualityChange = Item.isConjured(item) ? 2 * QUALITY_CHANGE : QUALITY_CHANGE;
            ...
            if (Item.isExpired(item)) {
              updateQualityOfExpired(item, qualityChange);
            }
        }
    }

    private void updateQuality(Item item, int qualityChange) {
    ...
    private void updateQualityOfExpired(Item item, int qualityChange) {
